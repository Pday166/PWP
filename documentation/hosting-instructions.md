...
- Create -> Droplet
    ...
    - Choose hostname (internal to docker, e.g. pday5-pwp)
...
- ssh into the server
    - For Mac and Linux: `ssh root@patday.me`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo pday5`
    - replace pday5 with your own
- create password
    - `passwd pday5`
- Move .ssh key file
    - `cp -r .ssh /home/pday5`
- change ownership of the .ssh directory
    - `cd /home/pday5`
    - `chown -R pday5:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh pday5@patday.me`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
        - **Connection → Data**
            - Auto-login pday5: change from root to pday5
        - **Session**
            - Click pwp
            - Click Save
...
- create a directory for your deployment
    - `sudo mkdir /var/www/patday.me/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/patday.me/html`
...
- `sudo vim /etc/nginx/sites-available/patday.me`
...
server {
    ...
    root /var/www/patday.me/html;
    index index.html index.htm index.nginx-debian.html;

    server_name patday.me www.patday.me;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/patday.me /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/pday5/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/pday5/.ssh/github_deploy_key.pub >> /home/pday5/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/pday5/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H patday.me >> ~/.ssh/known_hosts 
         scp -r ./dist/* pday5@patday.me:/var/www/patday.me/html
```
